"""
Command Line Interface for Voice-to-Text Whisper Agent pipelined into LLM and Kokoro TTS with Silero VAD.
"""

import sys
import os
from dotenv import load_dotenv
from .recorder import check_microphone, record_audio_push_to_talk, record_audio_fixed, record_audio_vad
...
def print_banner():
    print("=" * 72)
    print("   🎙️  VOICE AGENT 2: SILERO VAD ➔ WHISPER ➔ LLM ➔ KOKORO TTS PIPELINE 🔊")
    print("=" * 72)
...
def main():
    ...
    while True:
        ...
        print("\nChoose an option:")
        print(" [1] ⚡ Hands-Free Auto VAD Mode (Silero VAD Speech Detection + Interruption)")
        print(" [2] 🔴 Push-to-Talk Mode (Press Enter to Start/Stop)")
        print(" [3] ⏱️  Fixed Timer Mode (Record for 5 seconds)")
        print(" [4] 📁 Transcribe Audio File -> LLM -> Kokoro TTS")
        print(f" [5] 🔊 Toggle TTS Voice Output (Currently: {tts_status})")
        print(f" [6] ⚙️  Settings (Whisper: {whisper_model} | LLM: {llm_engine.provider.upper()})")
        print(" [7] 🧹 Clear LLM Conversation Context")
        print(" [8] ❌ Exit")
        ...
        if choice == "1":
            try:
                ...
                audio = detector.record_audio_vad(silence_duration=1.0, tts_engine=tts_engine)
                if len(audio) == 0:
                ...
                print(f"❌ Error during VAD voice pipeline: {e}")
            ...
            print(f" Current VAD Engine:   Silero VAD 6.2.1")
